...
parser.add_argument('-t','--runtype', help='desired type of test', default='run_single')
...
while True:
    ...
    elif args.runtype == 'run_pow_sweep':
        if not (args.start and args.stop and args.step):
            print ("Power sweep arguments not provided! Aborting operation...")
            raise KeyboardInterrupt
        pow_sweep_args = [args.metadata, args.datapath, args.start, args.stop, args.step, '-e' ,args.endpoint, '-s']
        if args.allboards:
